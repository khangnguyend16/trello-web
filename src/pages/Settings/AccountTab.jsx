...
function AccountTab() {
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    toast.promise(dispatch(updateUserAPI({ displayName })), { pending: "Updating..." }).then((res) => {
      // Phải kiểm tra ko có lỗi (update thành công) thì mới thực hiện các hành động cần thiết
      if (!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    // // log được dữ liệu thông qua FormData
    ...
    toast.promise(dispatch(updateUserAPI(reqData)), { pending: "Updating..." }).then((res) => {
      ...
      // Dù có lỗi hay không thì cũng phải clear lại input file
      e.target.value = "";
    });
  };
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box>
            <Avatar sx={{ width: 84, height: 84, mb: 1 }} alt="KhangNguyen" src={currentUser?.avatar} />
